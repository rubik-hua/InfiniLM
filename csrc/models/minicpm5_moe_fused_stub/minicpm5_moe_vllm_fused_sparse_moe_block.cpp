...
#include "../minicpm5_moe/minicpm5_moe_router_cpu_detail.hpp"
...
infinicore::Tensor MiniCPM5MoeVllmFusedSparseMoeBlock::forward(const infinicore::Tensor &hidden_states) const {
    ...
    if (const char *dbg = std::getenv("INFINILM_DEBUG_VLLM_FUSED_MOE")) {
        if (std::string(dbg) == "1") {
            ...
            if (!printed) {
                printed = true;
                std::fprintf(stderr, "[INFINILM_DEBUG_VLLM_FUSED_MOE] enter moe forward, fused_avail=%d\n", fused_avail ? 1 : 0);
                std::fflush(stderr);
            }
        }
    }
    ...
    infinilm::models::minicpm5_moe::router_cpu_detail::RouterTopkCpuResult router_out;
    infinilm::models::minicpm5_moe::router_cpu_detail::run_router_topk_cpu(
        logits_cpu,
        bias_cpu,
        n_tokens,
        n_routed_experts,
    ...
    auto topk_w_cpu =
        infinicore::Tensor::empty({n_tokens, top_k}, act_dt, infinicore::Device::cpu());
    for (size_t t = 0; t < n_tokens; ++t) {
        for (size_t j = 0; j < top_k; ++j) {
            infinilm::models::minicpm5_moe::router_cpu_detail::write_f32_as_element(
                topk_w_cpu, t * top_k + j, topk_weights_cpu[t][j]);
        }
    }
    ...
    if (!routed_opt.has_value()) {
        if (const char *dbg = std::getenv("INFINILM_DEBUG_VLLM_FUSED_MOE")) {
            if (std::string(dbg) == "1") {
                ...
                if (!printed) {
                    printed = true;
                    std::fprintf(stderr, "[INFINILM_DEBUG_VLLM_FUSED_MOE] routed_opt=nullopt -> fallback to reference MoE\n");
                    std::fflush(stderr);
                }
            }
        }
    ...
    if (const char *dbg = std::getenv("INFINILM_DEBUG_VLLM_FUSED_MOE")) {
        if (std::string(dbg) == "1") {
            ...
            if (!printed) {
                printed = true;
                std::fprintf(stderr, "[INFINILM_DEBUG_VLLM_FUSED_MOE] routed_opt=OK -> using vLLM fused experts\n");
                std::fflush(stderr);
            }
        }
    }
    ...
    return infinicore::op::add(routed, shared);
        ...
        if (const char *dbg = std::getenv("INFINILM_DEBUG_VLLM_FUSED_MOE")) {
            if (std::string(dbg) == "1") {
                ...
                if (!printed) {
                    printed = true;
                    std::fprintf(stderr, "[INFINILM_DEBUG_VLLM_FUSED_MOE] exception in fused path -> fallback to reference MoE\n");
                    std::fflush(stderr);
                }
            }
        }
