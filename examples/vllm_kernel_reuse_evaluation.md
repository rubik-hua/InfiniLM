...
**InfiniLM (current):** the MiniCPM5-MoE model uses `model_type` **`minicpm5_moe`** only. The MoE path uses **`MiniCPM5MoeVllmFusedSparseMoeBlock`**: HF-aligned CPU routing, then **`infinicore.vllm_fused_moe_bridge.fused_experts_ic`** (via `InfiniLM/csrc/utils/vllm_fused_moe_dispatch.cpp`, GIL + PyBind) when vLLM/ATen are importable; otherwise it falls back to the per-expert C++ loop in `MiniCPM5MoeSparseMoeBlock`. Set **`INFINILM_FORCE_MOE_BACKEND=baseline`** (or `INFINILM_DISABLE_VLLM_FUSED_MOE=1`) to force the reference path; set **`INFINILM_FORCE_MOE_BACKEND=vllm_fused`** to require the fused path.
