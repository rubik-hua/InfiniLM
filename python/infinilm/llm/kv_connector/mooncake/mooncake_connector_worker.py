...
from .mooncake_connector_utils import get_ip, make_zmq_path, EngineId, make_zmq_socket
...
def should_launch_bootstrap_server(infinilm_config) -> bool:
    assert (parallel_config := infinilm_config.parallel_config)
...
def get_mooncake_bootstrap_addr(infinilm_config) -> tuple[str, int]:
    ...
    port = VLLM_MOONCAKE_BOOTSTRAP_PORT
    return (host, port)
...
class MooncakeConnectorWorker:
    ...
    def __init__(self, infinilm_config, engine_id: str):
        ...
        assert (kv_transfer_config := infinilm_config.kv_transfer_config)
        ...
        self.tp_rank = (
            infinilm_config.parallel_config.tensor_parallel_rank
        )  # TODO: 再确认
        self.tp_size = (
            infinilm_config.parallel_config.tensor_parallel_size
        )  # TODO: 再确认
        self.num_blocks = 0

        assert (parallel_config := infinilm_config.parallel_config)

        dp_rank = 0  # parallel_config.data_parallel_index  # TODO: 再确认
        dp_local_rank = 0  # parallel_config.data_parallel_rank_local  # TODO: 再确认
        # self.dp_rank = (
        #     dp_local_rank if parallel_config.local_engines_only else dp_rank
        # )  # TODO: 再确认

        self.dp_rank = dp_local_rank

        pp_size = (
            1  # infinilm_config.parallel_config.pipeline_parallel_size  # TODO: 再确认
        )
        if pp_size > 1:
            raise ValueError(
                "Mooncake Transfer Engine does not support pipeline parallelism yet."
            )
        self.pp_rank = 0  # ().rank_in_group  #  TODO: 再确认
        ...
        if not self.is_kv_consumer:
            ...
            if should_launch_bootstrap_server(infinilm_config):
                _, port = get_mooncake_bootstrap_addr(infinilm_config)
                self.bootstrap_server = MooncakeBootstrapServer(
                    infinilm_config, "0.0.0.0", port
                )
        ...
        self.block_size = infinilm_config.block_size
        self.use_mla = False
        ...
        self.kv_topo = TpKVTopology(
            ...
            is_mla=self.use_mla,
            total_num_kv_heads=infinilm_config.get_total_num_kv_heads(),
        )
